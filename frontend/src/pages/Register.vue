    ...
    async onRegister() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          this.error = data.detail || 'Registration failed. Please try a different username.';
        } else {
          const data = await res.json();
          this.success = 'Registration successful! Logging in...';
    ...
    async autoLogin() {
      try {
        ...
        if (res.ok) {
          ...
          this.success = 'Registration and login successful! Redirecting to dashboard...';
          setTimeout(() => {
            this.$router.push('/');
          }, 1500);
        } else {
          this.success = 'Registration successful! Please login manually.';
          setTimeout(() => {
